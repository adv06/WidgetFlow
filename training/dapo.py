...
def advantages(rewards, eps=1e-6):
    # group-normalised, shared across a sequence's tokens (the "Advantages" box)
...
def dapo_loss(policy, scored, *, clip_low=0.2, clip_high=0.28, std_threshold=0.0,
    ...
    if r.std(unbiased=False) <= std_threshold:            # DAPO dynamic sampling
        return None
    ...
    for a, x in zip(adv, scored):
        ro = x.rollout
        ratio = torch.exp(policy.logprob(ro) - ro.old_logprob)     # (T,) differentiable
        a = a.to(ratio.device)
        per_tok = -torch.min(ratio * a, ratio.clamp(1 - clip_low, 1 + clip_high) * a)
        if credit is not None:
            per_tok = per_tok * credit.get(ro, 1.0)                # per-token credit weights
        num = num + per_tok.sum()
        ntok += per_tok.numel()
